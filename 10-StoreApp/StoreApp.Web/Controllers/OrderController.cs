...
{
    private Cart cart;
    private IOderRepository _orderRepository;

    public OrderController(Cart cartService, IOderRepository orderRepository)
    {
        cart = cartService;
    ...
    public IActionResult Checkout(OrderModel model)
    {
        ...
        if (ModelState.IsValid)
        {
            var order = new Order
            {
                ...
                City = model.City,
                Phone = model.Phone,
                AdreesLine = model.AdreesLine,
                OrderData = DateTime.Now,
                OrderItem = cart.Items.Select(i => new StoreApp.Data.Concrete.OrderItem
                {
        ...
        private Payment ProcessPayment(OrderModel model)
        {
            ...
            // Iyzico sandbox test bilgileri. Canli ortamda bu degerleri
            // appsettings/user-secrets uzerinden kendi API bilgilerinizle kullanin.
            options.ApiKey = "sandbox-o5U6SrkMiTqDPXFcs1xKWNCMtTybhnaD".Trim();
            options.SecretKey = "1qeSR1aXscQFszw1Vn0lSE6tFAYDq3WC".Trim();
        ...
        Payment payment = Payment.Create(request, options).GetAwaiter().GetResult();
